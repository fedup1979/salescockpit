...
- Latest local validation for the SchoolDrive V1 business contract: `compileall` OK, `239 passed` with `.\.venv\Scripts\python.exe -m pytest --basetemp=.pytest-tmp\full-schooldrive-v1-final-2`, local `scripts/pre_cutover_check.py --allow-cold-prod` OK, Playwright local non-mutating suite `14 passed / 8 skipped`. On Windows, run pytest with `--basetemp=.pytest-tmp\run` if `%TEMP%\pytest-current` cleanup raises a permission error after successful execution.
...
- SchoolDrive V1 hard stops now documented for Pilotage: categories outside APP/FSM/AS, missing category, Roadmap/product-only, full sessions and same-category signed records stop structured follow-ups without creating automatic admin review; only inbound replies create a new operational action.
